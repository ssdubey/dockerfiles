...
3. Start and Run the containers with those images. (`Master` should be run only once and `worker` can be run as many times as required.
 All the workers will attach with the same master to create the cluster)
...
./script_worker
```
5. Run `ps` on master and worker nodes to check if ipfs and the service daemon is running.
